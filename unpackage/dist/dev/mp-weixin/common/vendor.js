  ...
  [
    {
      ...
      "provider": "aliyun",
      "spaceId": "mp-14a98f01-63d4-4ccd-9efa-9e6430f4ff57"
    }
  ]
...
const xt = "none", Rt = "request", Ut = "response", Lt = "both";
class Nt {
  constructor({ secretType: e2, uniCloudIns: t2 } = {}) {
    this.clientType = "", this.secretType = e2 || xt, this.uniCloudIns = t2;
  ...
  async encryptData(e2) {
    return this.secretType === xt ? e2 : this.platformEncryptData(e2);
  }
  async decryptResult(e2) {
    if (this.secretType === xt)
      return e2;
    const { errCode: t2, content: n2 } = e2 || {};
    return t2 || !n2 ? e2 : this.secretType === Rt ? n2 : this.platformDecryptResult(e2);
  }
  ...
  wrapEncryptDataCallFunction(e2) {
    ...
    return async function({ name: n2, data: s2 = {} } = {}) {
      ...
      if (t2.isClientKeyNotFound(i2)) {
        ...
        s2._uniCloudOptions = await t2.platformGetSignOption(), i2 = await e2({ name: n2, data: r3 });
      }
...
var jt, Bt = { toBytes: function(e2) {
  ...
  return t2.join("");
} }, $t = (jt = "0123456789abcdef", { toBytes: function(e2) {
...
class cn {
  constructor(e2) {
    ...
    Object.defineProperty(this, "key", { value: Mt(e2, true) }), this._prepare();
  }
  _prepare() {
    var e2 = Wt[this.key.length];
    if (null == e2)
      throw new Error("invalid key size (must be 16, 24 or 32 bytes)");
    this._Ke = [], this._Kd = [];
    for (var t2 = 0; t2 <= e2; t2++)
      this._Ke.push([0, 0, 0, 0]), this._Kd.push([0, 0, 0, 0]);
    var n2, s2 = 4 * (e2 + 1), r2 = this.key.length / 4, i2 = an(this.key);
    for (t2 = 0; t2 < r2; t2++)
      n2 = t2 >> 2, this._Ke[n2][t2 % 4] = i2[t2], this._Kd[e2 - n2][t2 % 4] = i2[t2];
    for (var o2, a2 = 0, c2 = r2; c2 < s2; ) {
      if (o2 = i2[r2 - 1], i2[0] ^= Jt[o2 >> 16 & 255] << 24 ^ Jt[o2 >> 8 & 255] << 16 ^ Jt[255 & o2] << 8 ^ Jt[o2 >> 24 & 255] ^ zt[a2] << 24, a2 += 1, 8 != r2)
        for (t2 = 1; t2 < r2; t2++)
          i2[t2] ^= i2[t2 - 1];
      else {
        for (t2 = 1; t2 < r2 / 2; t2++)
          i2[t2] ^= i2[t2 - 1];
        o2 = i2[r2 / 2 - 1], i2[r2 / 2] ^= Jt[255 & o2] ^ Jt[o2 >> 8 & 255] << 8 ^ Jt[o2 >> 16 & 255] << 16 ^ Jt[o2 >> 24 & 255] << 24;
    ...
    for (var u2 = 1; u2 < e2; u2++)
      for (var h2 = 0; h2 < 4; h2++)
        o2 = this._Kd[u2][h2], this._Kd[u2][h2] = nn[o2 >> 24 & 255] ^ sn[o2 >> 16 & 255] ^ rn[o2 >> 8 & 255] ^ on[255 & o2];
  }
  encrypt(e2) {
    if (16 != e2.length)
      throw new Error("invalid plaintext size (must be 16 bytes)");
    for (var t2 = this._Ke.length - 1, n2 = [0, 0, 0, 0], s2 = an(e2), r2 = 0; r2 < 4; r2++)
      s2[r2] ^= this._Ke[0][r2];
    for (var i2 = 1; i2 < t2; i2++) {
      for (r2 = 0; r2 < 4; r2++)
        n2[r2] = Gt[s2[r2] >> 24 & 255] ^ Vt[s2[(r2 + 1) % 4] >> 16 & 255] ^ Yt[s2[(r2 + 2) % 4] >> 8 & 255] ^ Qt[255 & s2[(r2 + 3) % 4]] ^ this._Ke[i2][r2];
      s2 = n2.slice();
    }
    var o2, a2 = qt(16);
    for (r2 = 0; r2 < 4; r2++)
      o2 = this._Ke[t2][r2], a2[4 * r2] = 255 & (Jt[s2[r2] >> 24 & 255] ^ o2 >> 24), a2[4 * r2 + 1] = 255 & (Jt[s2[(r2 + 1) % 4] >> 16 & 255] ^ o2 >> 16), a2[4 * r2 + 2] = 255 & (Jt[s2[(r2 + 2) % 4] >> 8 & 255] ^ o2 >> 8), a2[4 * r2 + 3] = 255 & (Jt[255 & s2[(r2 + 3) % 4]] ^ o2);
    return a2;
  }
  decrypt(e2) {
    if (16 != e2.length)
      throw new Error("invalid ciphertext size (must be 16 bytes)");
    for (var t2 = this._Kd.length - 1, n2 = [0, 0, 0, 0], s2 = an(e2), r2 = 0; r2 < 4; r2++)
      s2[r2] ^= this._Kd[0][r2];
    for (var i2 = 1; i2 < t2; i2++) {
      for (r2 = 0; r2 < 4; r2++)
        n2[r2] = Xt[s2[r2] >> 24 & 255] ^ Zt[s2[(r2 + 3) % 4] >> 16 & 255] ^ en[s2[(r2 + 2) % 4] >> 8 & 255] ^ tn[255 & s2[(r2 + 1) % 4]] ^ this._Kd[i2][r2];
      s2 = n2.slice();
    }
    var o2, a2 = qt(16);
    for (r2 = 0; r2 < 4; r2++)
      o2 = this._Kd[t2][r2], a2[4 * r2] = 255 & (Ht[s2[r2] >> 24 & 255] ^ o2 >> 24), a2[4 * r2 + 1] = 255 & (Ht[s2[(r2 + 3) % 4] >> 16 & 255] ^ o2 >> 16), a2[4 * r2 + 2] = 255 & (Ht[s2[(r2 + 2) % 4] >> 8 & 255] ^ o2 >> 8), a2[4 * r2 + 3] = 255 & (Ht[255 & s2[(r2 + 1) % 4]] ^ o2);
    return a2;
  }
}
class un {
  constructor(e2) {
    if (!(this instanceof un))
      throw Error("AES must be instanitated with `new`");
    this.description = "Electronic Code Block", this.name = "ecb", this._aes = new cn(e2);
  }
  encrypt(e2) {
    if ((e2 = Mt(e2)).length % 16 != 0)
      throw new Error("invalid plaintext size (must be multiple of 16 bytes)");
    for (var t2 = qt(e2.length), n2 = qt(16), s2 = 0; s2 < e2.length; s2 += 16)
      Kt(e2, n2, 0, s2, s2 + 16), Kt(n2 = this._aes.encrypt(n2), t2, s2);
    return t2;
  }
  decrypt(e2) {
    if ((e2 = Mt(e2)).length % 16 != 0)
    ...
    for (var t2 = qt(e2.length), n2 = qt(16), s2 = 0; s2 < e2.length; s2 += 16)
      Kt(e2, n2, 0, s2, s2 + 16), Kt(n2 = this._aes.decrypt(n2), t2, s2);
    return t2;
  }
}
class hn {
  constructor(e2, t2) {
    ...
    if (this.description = "Cipher Block Chaining", this.name = "cbc", t2) {
      ...
      t2 = qt(16);
    this._lastCipherblock = Mt(t2, true), this._aes = new cn(e2);
  }
  encrypt(e2) {
    if ((e2 = Mt(e2)).length % 16 != 0)
      throw new Error("invalid plaintext size (must be multiple of 16 bytes)");
    for (var t2 = qt(e2.length), n2 = qt(16), s2 = 0; s2 < e2.length; s2 += 16) {
      ...
      this._lastCipherblock = this._aes.encrypt(n2), Kt(this._lastCipherblock, t2, s2);
    }
    return t2;
  }
  decrypt(e2) {
    if ((e2 = Mt(e2)).length % 16 != 0)
      throw new Error("invalid ciphertext size (must be multiple of 16 bytes)");
    for (var t2 = qt(e2.length), n2 = qt(16), s2 = 0; s2 < e2.length; s2 += 16) {
      Kt(e2, n2, 0, s2, s2 + 16), n2 = this._aes.decrypt(n2);
      for (var r2 = 0; r2 < 16; r2++)
        t2[s2 + r2] = n2[r2] ^ this._lastCipherblock[r2];
      Kt(e2, this._lastCipherblock, 0, s2, s2 + 16);
    }
    return t2;
  }
}
class ln {
  constructor(e2, t2, n2) {
    ...
    if (this.description = "Cipher Feedback", this.name = "cfb", t2) {
      ...
      t2 = qt(16);
    n2 || (n2 = 1), this.segmentSize = n2, this._shiftRegister = Mt(t2, true), this._aes = new cn(e2);
  }
  encrypt(e2) {
    if (e2.length % this.segmentSize != 0)
      throw new Error("invalid plaintext size (must be segmentSize bytes)");
    for (var t2, n2 = Mt(e2, true), s2 = 0; s2 < n2.length; s2 += this.segmentSize) {
      t2 = this._aes.encrypt(this._shiftRegister);
      for (var r2 = 0; r2 < this.segmentSize; r2++)
        n2[s2 + r2] ^= t2[r2];
      Kt(this._shiftRegister, this._shiftRegister, 0, this.segmentSize), Kt(n2, this._shiftRegister, 16 - this.segmentSize, s2, s2 + this.segmentSize);
    }
    return n2;
  }
  decrypt(e2) {
    ...
    for (var t2, n2 = Mt(e2, true), s2 = 0; s2 < n2.length; s2 += this.segmentSize) {
      ...
      for (var r2 = 0; r2 < this.segmentSize; r2++)
        n2[s2 + r2] ^= t2[r2];
      Kt(this._shiftRegister, this._shiftRegister, 0, this.segmentSize), Kt(e2, this._shiftRegister, 16 - this.segmentSize, s2, s2 + this.segmentSize);
    }
    return n2;
  }
}
class dn {
  constructor(e2, t2) {
    ...
    if (this.description = "Output Feedback", this.name = "ofb", t2) {
      ...
      t2 = qt(16);
    this._lastPrecipher = Mt(t2, true), this._lastPrecipherIndex = 16, this._aes = new cn(e2);
  }
  encrypt(e2) {
    for (var t2 = Mt(e2, true), n2 = 0; n2 < t2.length; n2++)
...
class pn {
  constructor(e2) {
    ...
    0 === e2 || e2 || (e2 = 1), "number" == typeof e2 ? (this._counter = qt(16), this.setValue(e2)) : this.setBytes(e2);
  }
  setValue(e2) {
...
class fn {
  constructor(e2, t2) {
    if (!(this instanceof fn))
      throw Error("AES must be instanitated with `new`");
    this.description = "Counter", this.name = "ctr", t2 instanceof pn || (t2 = new pn(t2)), this._counter = t2, this._remainingCounter = null, this._remainingCounterIndex = 16, this._aes = new cn(e2);
  }
  encrypt(e2) {
    for (var t2 = Mt(e2, true), n2 = 0; n2 < t2.length; n2++)
...
var gn = { AES: cn, Counter: pn, ModeOfOperation: { ecb: un, cbc: hn, cfb: ln, ofb: dn, ctr: fn }, utils: { hex: $t, utf8: Bt }, padding: { pkcs7: { pad: function(e2) {
  var t2 = 16 - (e2 = Mt(e2, true)).length % 16, n2 = qt(e2.length + t2);
  Kt(e2, n2);
  for (var s2 = e2.length; s2 < n2.length; s2++)
    n2[s2] = t2;
  return n2;
}, strip: function(e2) {
  if ((e2 = Mt(e2, true)).length < 16)
  ...
  var r2 = qt(n2);
  return Kt(e2, r2, 0, 0, n2), r2;
} } }, _arrayTest: { coerceArray: Mt, createArray: qt, copyArray: Kt } };
function mn(e2, t2, n2) {
  const s2 = new Uint8Array(index.base64ToArrayBuffer(t2)), r2 = gn.utils.utf8.toBytes(n2), i2 = gn.utils.utf8.toBytes(e2), o2 = new gn.ModeOfOperation.cbc(s2, r2), a2 = gn.padding.pkcs7.pad(i2), c2 = o2.encrypt(a2);
  return index.arrayBufferToBase64(c2);
}
const yn = { code: 2e4, message: "System error" }, _n = { code: 20101, message: "Invalid client" }, wn = { code: 20102, message: "Get encrypt key failed" };
function In(e2) {
...
class Cn extends Nt {
  ...
  async getUserEncryptKey() {
    ...
    if (kn && kn.expireTime) {
      const e2 = Date.now();
      if (kn.expireTime - e2 > 0)
        return this.userEncryptKey = kn, this.userEncryptKey;
    }
    return new Promise((e2, t2) => {
      index.getUserCryptoManager().getLatestUserKey({ success: (t3) => {
        kn = t3, this.userEncryptKey = t3, e2(this.userEncryptKey);
      }, fail: (e3) => {
        t2(In({ ...wn, cause: e3 }));
      } });
    });
  ...
  async platformGetSignOption() {
    ...
    return { verifyClientSign: mn(JSON.stringify({ data: JSON.stringify({}), appId: this.appId, deviceId: this.deviceId, wxAppId: this.getWxAppId(), simulator: "devtools" === ue().platform, timestamp: Date.now() }), e2, t2), encryptKeyId: n2, mpWeixinCode: this.scopedGlobalCache.mpWeixinCode, mpWeixinOpenid: this.scopedGlobalCache.mpWeixinOpenid };
  }
  async platformEncryptData(e2) {
    ...
    return this.secretType === Ut ? { content: e2, _uniCloudOptions: r2 } : { content: mn(JSON.stringify({ data: JSON.stringify(e2), appId: this.appId, deviceId: this.deviceId, wxAppId: this.getWxAppId(), simulator: "devtools" === ue().platform, timestamp: Date.now() }), t2, n2), _uniCloudOptions: r2 };
  }
  async platformDecryptResult(e2) {
    ...
    return JSON.parse(function(e3, t3, n3) {
      const s3 = new Uint8Array(index.base64ToArrayBuffer(e3)), r2 = new Uint8Array(index.base64ToArrayBuffer(t3)), i2 = gn.utils.utf8.toBytes(n3), o2 = new gn.ModeOfOperation.cbc(r2, i2), a2 = gn.padding.pkcs7.strip(o2.decrypt(s3));
      return gn.utils.utf8.fromBytes(a2);
    }(t2, n2, s2));
  }
  isClientKeyNotFound() {
    return false;
